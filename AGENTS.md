...
- `make -C charts/monitoring all` — regenerate PrometheusRules + GrafanaDashboards from the jsonnet mixins (targets: `clean`, `alerts`, `dashboards`, `all`; requires `jsonnet` and `yq`). Generated files `charts/monitoring/prometheus/alerts.yaml` and `charts/monitoring/grafana-operator/dashboards.yaml` carry a "DO NOT EDIT" header; `charts/monitoring/manifests/` output is gitignored.
...
- Stack in the `monitoring` namespace: VictoriaMetrics, Prometheus, Grafana operator, kube-state-metrics, node-exporter.
...
- **Generated files** — anything carrying a "DO NOT EDIT" header: `charts/monitoring/prometheus/alerts.yaml`, `charts/monitoring/grafana-operator/dashboards.yaml`, plus the gitignored `charts/monitoring/manifests/`. Regenerate via `make -C charts/monitoring ...` instead of editing.
